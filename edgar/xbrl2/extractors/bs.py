#!/usr/bin/env python3
"""
Standardized Balance Sheet Extraction
Extracts balance sheet using balance-sheet.json mapping.
"""
...
def try_balance_sheet_df(company: Company, form: str) -> tuple[Optional[pd.DataFrame], Optional[str]]:
    ...
    if hasattr(xbrl, 'current_period'):
        try:
            stmt_df = xbrl.current_period.balance_sheet(
                as_statement=False,
    ...
    if form == "10-K":
        try:
            ...
            if fin is not None:
                stmt = getattr(fin, "balance_sheet", None)
                if callable(stmt):
...
def main():
    ...
    df_stmt, filing_period_end = try_balance_sheet_df(company, form=args.form)
