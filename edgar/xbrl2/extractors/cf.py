#!/usr/bin/env python3
"""
Standardized Cash Flow Extraction
Extracts balance sheet using cash-flow.json mapping.
"""
...
def try_cash_flow_df(company: Company, form: str) -> tuple[Optional[pd.DataFrame], Optional[str]]:
    ...
    if hasattr(xbrl, 'current_period'):
        try:
            stmt_df = xbrl.current_period.cashflow_statement(
                as_statement=False,
    ...
    if form == "10-K":
        try:
            ...
            if fin is not None:
                stmt = getattr(fin, "cashflow_statement", None)
                if callable(stmt):
...
def main():
    ...
    default_mapping = script_dir.parent / "schemas" / "cash-flow.json"
    ...
    df_stmt, filing_period_end = try_cash_flow_df(company, form=args.form)
